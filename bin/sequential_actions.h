...
class SequentialActions : public Actions {
	...
	void createFSCProgrammingActions( unsigned procedures, const StringVec &accumulator_preds, const StringVec &evaluating_preds, const StringDVec &tacts, const StringDVec &facts ){
		...
		addEffect( name, true_no_act_pred, false, incvec( 0, 1 ) );

		// Add instruction
		addActionToInstruction( name, true_no_act_pred );
		...
		addEffect( name, false_no_act_pred, false, incvec( 0, 1 ) );

		// Add instruction
		addActionToInstruction( name, false_no_act_pred  );
	...
	void createRepeatFSCNoAct( const String &name, int proc, bool is_true, const StringVec &accumulator_preds, const StringVec &evaluating_preds, const StringVec &action_preds){
		...
		if( is_true ){
			...
			addPrecondition( name, true_no_act_pred, false, incvec( 0, 1 ) );
		}
		else{
			addPrecondition( name, accumulator_preds[ proc ], true, IntVec( 1, 1 ) );
			addPrecondition( name, false_no_act_pred, false, incvec( 0, 1 ) );
		}
	...
	void createRepeatFSCActions( unsigned procedures, const StringVec &accumulator_preds, const StringVec &evaluating_preds, const StringVec &action_preds, const StringDVec &tacts, const StringDVec &facts ){ 
		// Repeat no actions
		for( unsigned p = 0; p < procedures + 1; p++ ){
			SStream ss;
			ss << "REPEAT-TRUE-NO-ACT-" << p;
			createRepeatFSCNoAct( ss.str() , p, true, accumulator_preds, evaluating_preds, action_preds );
			ss.str( "" );
			ss << "REPEAT-FALSE-NO-ACT-" << p;
			createRepeatFSCNoAct( ss.str() , p, false, accumulator_preds, evaluating_preds, action_preds );
		}
		// Repeat actions
