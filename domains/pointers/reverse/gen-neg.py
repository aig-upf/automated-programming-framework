...
str_problem=""
str_problem=str_problem + "(DEFINE (PROBLEM PNEG-" + str( size ) + str( lettername ) + ")\n"
...
str_problem = str_problem + "\n\t( AND"
for idx in xrange( 0, size ):
   str_problem = str_problem + "\n\t( VECTOR-ASSIGNMENT V" + str( idx ) + " V" + str( v[ random.randint(0, len( v ) - 1) ] ) + ")"
str_problem = str_problem + "\n\t)\n  )\n)"

str_problem = str_problem.lower()

f_problem=open( "pneg-" + str( size ) + str( lettername ) + ".pddl","w")
