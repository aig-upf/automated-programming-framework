...
str_problem=""
str_problem=str_problem + "(DEFINE (PROBLEM FIRST-LAST-INITIAL-"+str(name_size)+"-"+str(surname_size)+"-"+time.strftime("%d%m%Y%H%M%S")+")\n"
...
str_problem = str_problem + "\n\t( ASSIGNMENT RES I" + str( name_size ) + " SPACE )"
for idx in xrange( 0, 1 ):
   str_problem = str_problem + "\n\t( ASSIGNMENT RES I" + str( name_size + 1 + idx ) + " " + str( generated_str2[ idx ] ) + " )"
str_problem = str_problem + "\n\t( SIZE RES I" + str( name_size + 2 ) + " )"
str_problem=str_problem + "\n\t)\n  )\n)"
